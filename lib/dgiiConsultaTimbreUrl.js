...
/**
 * Cuentas con The Factory en **demo** emiten e-CF consultables solo en DGII **testecf**.
 * Si el cliente envía `ambiente: produccion` por error (p. ej. integración con env de prod),
 * el QR debe usar el mismo segmento que el comprobante real.
 * @param {"desarrollo"|"certificacion"|"produccion"} ambiente
 * @param {"demo"|"production"|undefined|null} theFactoryAmbienteKey — de `resolveTheFactoryUrlsForUser().ambienteKey`
 * @returns {"desarrollo"|"certificacion"|"produccion"}
 */
export function coerceDgiiQrAmbienteForTheFactoryAccount(ambiente, theFactoryAmbienteKey) {
  if (theFactoryAmbienteKey === "demo" && ambiente === "produccion") return "desarrollo";
  return ambiente;
}

/**
 * Ambiente DGII para `generar-qr` (integraciones con API key).
 * - Con `ambiente` en el body: prevalece lo enviado (p. ej. La Época en demo → `testecf`), más la corrección cuenta demo + body producción.
 * - Sin `ambiente`: se alinea al titular de la clave (`theFactoryAmbienteKey`), no a `NODE_ENV` del servidor Giganet.
 * @param {unknown} ambienteBody
 * @param {"demo"|"production"|undefined|null} theFactoryAmbienteKey
 * @returns {"desarrollo"|"certificacion"|"produccion"}
 */
export function resolveAmbienteQrParaGenerarQr(ambienteBody, theFactoryAmbienteKey) {
  const raw = String(ambienteBody ?? "").trim();
  if (raw === "") {
    if (theFactoryAmbienteKey === "demo") {
      return coerceDgiiQrAmbienteForTheFactoryAccount("desarrollo", theFactoryAmbienteKey);
    }
    if (theFactoryAmbienteKey === "production") {
      return coerceDgiiQrAmbienteForTheFactoryAccount("produccion", theFactoryAmbienteKey);
    }
  }
  return coerceDgiiQrAmbienteForTheFactoryAccount(resolveAmbienteQr(ambienteBody), theFactoryAmbienteKey);
}
...
export function normalizeFechaEmisionDdMmYyyy(input) {
  ...
  /** ISO solo fecha: evitar `new Date("YYYY-MM-DD")` (UTC medianoche → día anterior en TZ Americas). */
  const isoYmd = s.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (isoYmd) {
    const [, y, mo, da] = isoYmd;
    return { ok: true, value: `${da}-${mo}-${y}` };
  }
  const d = new Date(s);
...
/**
 * MontoTotal firmado en Totales del documento enviado a TheFactory (camelCase o PascalCase).
 * @param {unknown} facturaCompleta
 * @returns {string | null} monto con 2 decimales, o null
 */
export function montoTotalFirmadoFromDocumento(facturaCompleta) {
  if (!facturaCompleta || typeof facturaCompleta !== "object") return null;
  const doc = /** @type {Record<string, unknown>} */ (facturaCompleta).DocumentoElectronico;
  if (!doc || typeof doc !== "object") return null;
  const enc = /** @type {Record<string, unknown>} */ (doc).Encabezado;
  if (!enc || typeof enc !== "object") return null;
  const tot = /** @type {Record<string, unknown>} */ (enc).Totales;
  if (!tot || typeof tot !== "object") return null;
  const raw =
    /** @type {Record<string, unknown>} */ (tot).montoTotal ??
    /** @type {Record<string, unknown>} */ (tot).MontoTotal;
  return formatMontoTotalTwoDecimals(raw);
}

/**
 * Lee MontoTotal de una URL ConsultaTimbre / ConsultaTimbreFC.
 * @param {unknown} url
 * @returns {string | null}
 */
export function montoTotalFromConsultaTimbreUrl(url) {
  const s = String(url ?? "").trim();
  if (!s) return null;
  try {
    const u = new URL(s);
    return formatMontoTotalTwoDecimals(u.searchParams.get("MontoTotal"));
  } catch {
    return null;
  }
}
 ...
 *   facturaOriginal: Record<string, unknown>;
 *   theFactoryAmbienteKey?: "demo"|"production";
 *   montoTotalFirmado?: number | string | null;
 * }}
 * @returns {{ ok: true; url: string } | { ok: false; message: string }}
 */
export function generateDGIIQRUrlFromEnvioResponse({
  responseData,
  facturaOriginal,
  theFactoryAmbienteKey,
  montoTotalFirmado = null,
}) {
  const fact = facturaOriginal?.factura && typeof facturaOriginal.factura === "object"
    ...
    : {};
  const montoFirmadoStr = formatMontoTotalTwoDecimals(montoTotalFirmado);
  const montoTotalConItbis =
    montoFirmadoStr != null
      ? Number.parseFloat(montoFirmadoStr)
      : Number.parseFloat(String(fact.total ?? "0").replace(/,/g, "."));
  const tipoComprobante = String(fact.tipo ?? "");
  const ambiente = resolveAmbienteQrParaGenerarQr(facturaOriginal?.ambiente, theFactoryAmbienteKey);
  ...
  if (String(tipoComprobante) === "32") {
    // ConsultaTimbreFC exige MontoTotal del e-CF (= total a pagar con ITBIS), no el subtotal sin impuestos.
