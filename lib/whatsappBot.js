...
import {
  explicarValidacionCorta,
  looksLikeTheFactoryErrorPayload,
  normalizeErrorPayloadFromText,
...
export async function handleWhatsAppBotText(from, text) {
  ...
  // JSON / payload de error pegado → explicar aunque no traiga NCF
  if (looksLikeTheFactoryErrorPayload(raw)) {
    await handleExplicarError(from, {
      ncf: session?.lastNcf || null,
      rnc: session?.lastRnc || null,
      errorMessage: raw,
    });
    return;
  }

  if (session?.intent === "explicar_error" && session.step === "awaiting_details") {
    await handleExplicarError(from, {
      ncf: session.lastNcf || session.ncf || null,
      rnc: session.lastRnc || session.rnc || null,
      errorMessage: raw,
    });
    return;
  }
...
async function handleExplicarError(from, params) {
  const session = await getWhatsAppSession(from);
  let ncf = params.ncf ? String(params.ncf).trim().toUpperCase() : "";
  let rnc = params.rnc ? digitsOnly(params.rnc) : "";
  const errorMessage = params.errorMessage ? String(params.errorMessage).trim() : "";

  if (!ncf && session?.lastNcf) ncf = String(session.lastNcf).toUpperCase();
  if (!rnc && session?.lastRnc) rnc = digitsOnly(session.lastRnc);

  // 1) Si el usuario pegó un error con detalle, explícalo ya (con o sin NCF)
  if (errorMessage && looksLikeTheFactoryErrorPayload(errorMessage)) {
    const normalized = normalizeErrorPayloadFromText(errorMessage);
    const corta = explicarValidacionCorta(normalized);
    if (corta) {
      await clearWhatsAppSession(from);
      if (ncf || isValidRnc(rnc)) {
        await touchWhatsAppContext(from, {
          lastNcf: ncf || "",
          lastRnc: isValidRnc(rnc) ? rnc : "",
        });
      }
      const prefix = ncf ? `Sobre *${ncf}*:\n` : "";
      await sendWhatsAppTextMessage(from, prefix + corta);
      return;
    }
  }
  ...
  if (!ncf) {
    await setWhatsAppSession(from, {
      intent: "explicar_error",
      step: "awaiting_details",
      rnc: isValidRnc(rnc) ? rnc : "",
      ncf: "",
      lastNcf: session?.lastNcf || "",
      lastRnc: isValidRnc(rnc) ? rnc : session?.lastRnc || "",
    });
    await sendWhatsAppTextMessage(
      from,
      "Para explicarte un error necesito el *NCF del comprobante*.\nEjemplo: E320000000001\n\nTambién puedes copiarme el mensaje de error completo (el JSON con *errors*).",
    );
    return;
  }
  ...
  try {
    ...
    if (!auth || !auth.userId) {
      // Sin empresa: si hay mensaje de error, aún así intentar explicarlo
      if (errorMessage) {
        const normalized = normalizeErrorPayloadFromText(errorMessage);
        const corta = explicarValidacionCorta(normalized);
        if (corta) {
          await sendWhatsAppTextMessage(from, corta);
          return;
        }
        if (isWhatsAppBotAiConfigured()) {
          const explicacion = await explicarErrorConAI({
            errorMongoDB: null,
      ...
      await sendWhatsAppTextMessage(
        from,
        "No encontré esa empresa en Giganet. Pégame el *mensaje de error completo* (JSON) o indícame el *RNC del emisor*.",
      );
      return;
    }
    ...
    // Si hay payload detallado del usuario, priorizar explicación determinística
    if (errorMessage) {
      const normalized = normalizeErrorPayloadFromText(errorMessage);
      const corta = explicarValidacionCorta(normalized);
      if (corta) {
        await clearWhatsAppSession(from);
        await touchWhatsAppContext(from, { lastNcf: ncf, lastRnc: auth.emisorRnc || rnc || "" });
        await sendWhatsAppTextMessage(from, corta);
        return;
      }
    }

    // Si DB tiene validationErrors normalizados, responder directo
    if (errorDB?.errorNormalizado) {
      const corta = explicarValidacionCorta(errorDB.errorNormalizado);
      if (corta) {
        await clearWhatsAppSession(from);
        await touchWhatsAppContext(from, { lastNcf: ncf, lastRnc: auth.emisorRnc || rnc || "" });
        await sendWhatsAppTextMessage(from, corta);
    ...
    if (isWhatsAppBotAiConfigured() && (errorDB || errorMessage)) {
      explicacion = await explicarErrorConAI({
        ...
        errorUsuario: errorMessage || null,
        ncf,
      });
    } else {
    ...
    const mensajeDb = String(errorDB?.mensajeRespuesta || "");
    const esGenerico =
      !errorMessage &&
      !errorDB?.errorNormalizado?.validationErrors?.length &&
      (/Request failed with status code/i.test(mensajeDb) ||
        /sin indicar el motivo exacto/i.test(explicacion));

    if ((!errorDB && !errorMessage) || esGenerico) {
      await setWhatsAppSession(from, {
        intent: "explicar_error",
        step: "awaiting_details",
        rnc: auth.emisorRnc || rnc || "",
        ncf,
        lastNcf: ncf,
        lastRnc: auth.emisorRnc || rnc || "",
      });
      await sendWhatsAppTextMessage(
        from,
        "Si tienes el mensaje de error, cópialo completo (idealmente el JSON con *errors*) y envíamelo.",
      );
    }
  } catch (error) {
