...
/**
 * @typedef {{ intent: string, ncf: string | null, rnc: string | null, errorMessage: string | null }} BotInterpretation
 */
...
const interpretationSchema = z.object({
  ...
  errorMessage: z
    .string()
    .nullable()
    .describe("Mensaje de error copiado por el usuario si lo proporcionó"),
});
...
- Si pregunta por qué falló/error/rechazó un comprobante o pide ayuda con un error → explicar_error.
- Si el usuario copia un mensaje de error técnico o un JSON con "errors", extrae el errorMessage completo (todo el JSON) y usa intent=explicar_error. El NCF puede ser null si no viene en el mensaje.
- En el saludo inicial, el bot solo dice "¿Cómo te puedo ayudar?" (simple y directo).
- Si algo está fuera de alcance, el bot responde "Lo siento, no te puedo ayudar con eso" y lista las capacidades.
...
- Responde SOLO con el objeto estructurado.`;
  ...
  return {
    ...
    errorMessage: output.errorMessage ? String(output.errorMessage).trim() : null,
  };
}
...
function interpretHeuristic(text) {
  ...
  // Detectar si el usuario está copiando un mensaje de error
  if (
    /error|fallo|falló|rechaz|invalid|codigo|código|validation|DocumentoElectronico|excede la longitud/i.test(
      lower,
    ) &&
    (normalized.length > 50 ||
      /\{|\}|":|mensaje:|"errors"/i.test(normalized) ||
      /DocumentoElectronico\./i.test(normalized))
  ) {
  ...
  if (
    ...
    return { intent: "despedida", ncf: null, rnc: null, errorMessage: null };
  }
  ...
  if (
    ...
    return { intent: "cancelar", ncf: null, rnc: null, errorMessage: null };
  }

  // Detectar explicar error
  if (
    /\b(por\s*qu[eé].*error|error.*comprobante|fallo|falló|rechaz|me\s+dio\s+error)\b/i.test(
      lower,
    ) ||
    errorMessage
  ) {
    return { intent: "explicar_error", ncf, rnc, errorMessage };
  }

  // Fuera de alcance explícito (antes de matchear consultas)
  if (/\b(anular|cancelar\s+factura|eliminar)\b/i.test(lower)) {
    return { intent: "unknown", ncf, rnc, errorMessage: null };
  }
  ...
  if (
    ...
    return { intent: "consultar_estatus", ncf, rnc, errorMessage: null };
  }
  ...
  if (
    ...
    return { intent: "consultar_rnc", ncf: null, rnc, errorMessage: null };
  }
  ...
  if (ncf && !rnc) {
    return { intent: "consultar_estatus", ncf, rnc: null, errorMessage: null };
  }
  ...
  if (
    ...
    return { intent: "ayuda", ncf: null, rnc: null, errorMessage: null };
  }
