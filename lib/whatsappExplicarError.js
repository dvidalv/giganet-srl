...
export async function explicarErrorConAI({
  ...
  try {
    ...
    if (errorMongoDB) {
      ...
      if (normalizedError) {
        contextError = `Error registrado en el sistema para el comprobante ${ncf}:
...
**AUDIENCIA:** Hablas con usuarios que NO son programadores. Son personas que usan sistemas de facturación pero no conocen términos técnicos de programación o APIs.
...
5. **CUANDO NO HAY INFORMACIÓN DETALLADA:**
   - Si la API devolvió un error HTTP genérico (400, 500, etc.) pero NO proporciona detalles específicos (sin validationErrors, sin código The Factory, sin mensaje específico), entonces:
     * NO menciones el código HTTP (NO digas "400", "status code 400", "error 400")
     * NO digas "Request failed with status code..."
     * NO menciones términos técnicos de HTTP o APIs
     * Di simplemente: "El comprobante fue rechazado por el sistema, pero no se proporcionó información específica sobre el problema."
     * Luego sugiere revisar los datos más comunes que causan problemas (RNC, NCF, correo, montos, fechas)
     * Deja CLARO que son posibilidades generales, no la causa confirmada
     * Usa frases como: "Te recomiendo revisar estos datos comunes que suelen causar problemas..."
...
7. **LENGUAJE NO TÉCNICO:**
   - NUNCA uses términos como: API, endpoint, request, response, status code, JSON, validationErrors, HTTP
   - En lugar de "código de estado 400" di "el sistema rechazó el comprobante"
   - En lugar de "error de validación" di "hay un dato incorrecto"
   - En lugar de "revisar la respuesta de la API" di "revisa el mensaje de error"
   - NUNCA menciones "solicitud", "cliente", "servidor", "datos mal formateados"
   - Habla en lenguaje de facturación: comprobante, factura, datos del cliente, datos del negocio

8. **INSTRUCCIONES CLARAS Y ACCIONABLES:**
   - Di EXACTAMENTE dónde está el problema
   - Di EXACTAMENTE qué hay que cambiar
   - Proporciona ejemplos específicos del formato correcto
   - No digas "verifica que los datos sean correctos" - di QUÉ dato específico revisar

9. **DETECCIÓN DE ERRORES GENÉRICOS:**
   - Si el mensaje de error SOLO contiene algo como "HTTP Status: 400" y "Mensaje: Request failed with status code 400" sin más información:
     * Interpreta esto como "sin información específica"
     * NO repitas "Request failed with status code 400"
     * NO menciones el código 400
     * Responde según la regla 5 (cuando no hay información detallada)
...
1. Explica el error principal en español claro y simple (sin términos técnicos)
2. Si hay errores de validación, lista cada uno:
   - Número del error
   - Campo que tiene el problema (en español simple)
   - Qué está mal exactamente
   - Cómo corregirlo con ejemplo
3. Proporciona pasos específicos de solución que cualquier persona pueda seguir
4. Solo menciona hipótesis si la API no dio información específica

**EJEMPLO DE LO QUE NO DEBES HACER:**
❌ "El error *Request failed with status code 400* significa que la solicitud fue incorrecta..."
❌ "El código de estado 400 indica que la solicitud no se pudo procesar debido a un error del cliente..."
❌ "Es fundamental revisar la respuesta de la API para obtener detalles específicos..."
❌ "Validar los datos enviados: Verifica que todos los campos requeridos estén presentes..."
❌ "La solicitud fue incorrecta"
❌ "El sistema no pudo procesar la solicitud"
❌ Mencionar "HTTP", "status code", "código de estado", "response", "request", "API", "solicitud"

**EJEMPLO DE LO QUE SÍ DEBES HACER:**
✅ "El comprobante fue rechazado porque falta un dato obligatorio..."
✅ "Revisa que el correo electrónico del cliente esté completo..."
✅ "El RNC debe tener exactamente 9 u 11 números, por ejemplo: 101609921"
✅ Cuando no hay información específica: "El sistema rechazó el comprobante, pero no indicó exactamente qué dato causó el problema. Te recomiendo revisar estos datos comunes..."
